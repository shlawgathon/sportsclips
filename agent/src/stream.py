...
def stream_video_chunks(
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            raise RuntimeError(f"yt-dlp failed with return code {process.returncode}: {stderr_output}")
...
def stream_video_to_file(url: str, output_path: str, chunk_size: int = 1024 * 1024) -> None:
    """
    ...
    with open(output_path, 'wb') as f:
        for chunk in stream_video_chunks(url, chunk_size=chunk_size):
            f.write(chunk)
